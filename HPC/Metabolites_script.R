...
p_val_met <- rep(0, length(icc_met))

for (column in 1:length(icc_met)) {
  p_val_met[column] = sum(null_icc_matrix[column,] >= icc_met[column])/length(null_icc_matrix[column,])

}


###### Save p-values ######
saveRDS(p_val_met, 'pval_metabolites.rds')
