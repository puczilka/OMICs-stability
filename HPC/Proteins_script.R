...
work_dir<-"/rds/general/user/mmk4218/home/OMICS project"

### Put your local working directory here
#work_dir<-"/Users/sabrinar/Desktop/Teaching/Translational_Data_Science/Day4/Data_for_students/TDS_Session4/"
...
set.seed(1)
for (index in 1:9) {
  plot(density(log_proteins[, index], na.rm = TRUE), lwd = 2, col = "navy", main = "", las = 1, xlab = "", ylab = "")
}
...
colnames(icc_proteins) <- colnames(proteins)
...
p_val_proteins <- rep(0, length(icc_proteins))

for (column in 1:length(icc_proteins)) {
  p_val_proteins[column] = sum(null_icc_matrix[column,] >= icc_proteins[column])/length(null_icc_matrix[column,])

}
##### 7. Saving as an .rds object
ifelse(dir.exists("Results"),"",dir.create("Results"))
saveRDS(p_val_proteins,paste0(work_dir,"/Results/p_val_proteins.rds"))
